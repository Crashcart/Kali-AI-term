    ...
    try {
      ...
      if (fs.existsSync(sbxPath)) {
        try {
          ...
          this.logger?.warn(
            `Found Docker Sandboxes binary but version check failed: ${error.message}`
          );
        }
      }
  ...
  getInstallationInstructions() {
    const instructions = {
      ...
      win32: {
        ...
        commands: ['winget install Docker.sbx', 'docker-sbx --version'],
      },
      linux: {
        title: 'Install Docker Sandboxes on Linux',
        commands: ['docker plugin install docker/sbx:latest', 'docker-sbx --version'],
      },
    };
