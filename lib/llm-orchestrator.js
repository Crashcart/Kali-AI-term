...
class LLMOrchestrator extends EventEmitter {
  ...
  unregisterProvider(name) {
    ...
    delete this.stats.requestsByProvider[name];
    delete this.stats.errorsByProvider[name];
    this.logger?.info(`Unregistered LLM provider: ${name}`);
  ...
  async generate(prompt, options = {}) {
    ...
    const providers = preferredProvider
      ...
      : [strategy.primary, ...(Array.isArray(strategy.fallback)
          ? strategy.fallback
