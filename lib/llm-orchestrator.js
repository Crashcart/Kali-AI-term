...
class LLMOrchestrator extends EventEmitter {
  ...
  getDefaultRoutingStrategy() {
    return {
      primary: 'ollama', // Try local Ollama first (always available)
      fallback: 'gemini', // Fall back to Gemini if Ollama fails or is unavailable
      timeout: 30000,
      retries: 1,
    };
  }
  ...
  async generate(prompt, options = {}) {
    ...
    const providers = this._buildProviderList(preferredProvider, strategy);
      ...
      for (const providerName of providers) {
        try {
          ...
          this.stats.errorsByProvider[providerName] =
            (this.stats.errorsByProvider[providerName] || 0) + 1;
  ...
  async synthesizeResponses(prompt, options = {}) {
    const { providers = ['gemini', 'ollama'], systemPrompt = '', temperature = 0.7 } = options;
  ...
  /**
   * Build an ordered provider list for routing.
   * When a preferredProvider is given, it goes first followed by every other
   * registered provider.  Otherwise the strategy's primary and fallback are
   * tried first, then every remaining registered provider — so no healthy
   * instance is ever skipped.
   */
  _buildProviderList(preferredProvider, strategy) {
    if (preferredProvider) {
      return [preferredProvider, ...this._getFallbackProviders(preferredProvider)];
    }
    const ordered = [strategy.primary, ...(strategy.fallback ? [strategy.fallback] : [])];
    const rest = Array.from(this.providers.keys()).filter((p) => !ordered.includes(p));
    return [...ordered, ...rest];
  }
