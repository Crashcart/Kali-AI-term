...
class SandboxConfig {
  ...
  updateSandbox(sandboxId, updates) {
    ...
    if (updates.stoppedAt !== undefined) config.stoppedAt = updates.stoppedAt;

    if (updates.isolation) {
      Object.assign(config.isolation, updates.isolation);
    }
    if (updates.resources) {
      Object.assign(config.resources, updates.resources);
    }
    if (updates.security) {
      Object.assign(config.security, updates.security);
    }
    if (updates.execution) {
      Object.assign(config.execution, updates.execution);
    }
    if (updates.observability) {
      Object.assign(config.observability, updates.observability);
    }
  ...
  verifySandboxConstraints(sandboxId) {
    ...
    if (conflictingPaths.length > 0) {
      errors.push(
        `Allowed paths include restricted system directories: ${conflictingPaths.join(', ')}`
      );
    }

    // Verify timeout is reasonable
    if (config.execution.timeout < 1000 || config.execution.timeout > 86400000) {
      errors.push('Execution timeout out of range (1s - 24h)');
    }
