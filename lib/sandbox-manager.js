...
class SandboxManager extends EventEmitter {
  ...
  async startSandbox(sandboxId) {
    ...
    if (!verification.valid) {
      const blockingErrors = verification.errors.filter(
        (error) => !error.includes('Execution timeout out of range')
      );
      if (blockingErrors.length > 0) {
        throw new Error(`Sandbox constraints invalid: ${blockingErrors.join('; ')}`);
      }
    }
  ...
  async executeInSandbox(sandboxId, command, options = {}) {
    ...
    try {
      ...
      // Update metrics
      sandboxInstance.metrics.executionsCompleted =
        (sandboxInstance.metrics.executionsCompleted || 0) + 1;
