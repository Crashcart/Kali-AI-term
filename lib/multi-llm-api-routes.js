...
function createMultiLLMRoutes(orchestrator, logger) {
  ...
  router.post('/api/llm/generate', async (req, res) => {
    try {
      ...
      logger?.debug(`LLM Generate: taskType=${taskType}, provider=${preferredProvider}, model=${model}`);
  ...
  router.post('/api/llm/stream', async (req, res) => {
    try {
      ...
      logger?.debug(`LLM Stream: taskType=${taskType}, provider=${preferredProvider}, model=${model}`);
