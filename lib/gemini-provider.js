...
class GeminiProvider extends LLMProvider {
  constructor(apiKey, logger) {
    ...
    this.model = 'gemini-pro'; // or gemini-1.5-pro for advanced reasoning
    this.requestCount = 0;
    this.errorCount = 0;
  }
  ...
  async getModels() {
    try {
      ...
      const models = (response.data.models || [])
        .filter(m => m.name.includes('generative'))
        .map(m => ({
  ...
  /**
   * Stream response using Gemini (via polling)
   * Note: Google's streaming API requires different setup, so we'll use polling approach
   */
