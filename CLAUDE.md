# Claude Code Instructions — Kali-AI-term

## What This Repo Is

AI-powered terminal assistant with structured tool-calling patterns for Kali Linux penetration testing and security workflows.
...
- Branch naming: `type/short-description` (e.g., `feat/add-nmap-tool`, `fix/auth-crash`)
...
- Never commit credentials, API keys, or tokens — use `.env` or environment variables
- `.env.example` must exist and list every required env var with a comment explaining it

## Security Constraints

- This tool is for authorized testing and security research only
- Never generate payloads targeting systems without explicit authorization context
- Flag any request that looks like mass targeting, unauthorized scanning, or destructive use
...
Kali-AI-term/
├── scripts/
│   ├── check-rules-updates.sh  ← ai-rules update checker
│   └── *.sh                    ← tool-calling scripts
...
Security-focused repo — always verify authorization context before generating exploit code or tool payloads.
